...
function isInternalAudioNode(name, properties) {
  try {
    ...
    var appName = String(props["application.name"] || "").trim().toLowerCase()
    var binary = String(props["application.process.binary"] || "").trim().toLowerCase()
    var mediaName = String(props["media.name"] || "").trim().toLowerCase()
    return value === "quickshell"
      ...
      || value === "pw-record" || appName === "pw-record" || binary === "pw-record"
      || value === "cava" || appName === "cava" || binary === "cava" || mediaName === "cava"
      || value.indexOf("waveform") !== -1 || appName.indexOf("waveform") !== -1 || binary.indexOf("waveform") !== -1
      || value === "mpvpaper" || appName === "mpvpaper" || binary === "mpvpaper"
  } catch (e) {
    return false
  }
}
...
function uniqueRecordingStreamLabels(streams) {
  ...
  for (var i = 0; i < values.length && i < 512 && labels.length < 512; i++) {
    ...
    if (isIgnoredRecordingLabel(label)) continue
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    isIgnoredRecordingLabel: isIgnoredRecordingLabel,
