...
Panel {
  ...
  function isIgnoredRecordingLabel(label) {
    if (typeof Model.isIgnoredRecordingLabel === "function")
      return Model.isIgnoredRecordingLabel(label)
    if (!label) return true
    var value = String(label).trim().toLowerCase()
    return value === "pw-record"
      || value === "cava"
      || value === "mpvpaper"
      || value.indexOf("pw-record") !== -1
      || value.indexOf("cava") !== -1
      || value.indexOf("waveform") !== -1
      || value.indexOf("mpvpaper") !== -1
  }

  function observeRecordingApplications() {
    var current = listSnapshot(activeRecordingLabels)
    var rawAdditions = Model.addedRecordingStreamLabels(observedRecordingLabels, current)
    observedRecordingLabels = current
    var additions = []
    for (var i = 0; i < rawAdditions.length; i++) {
      if (!isIgnoredRecordingLabel(rawAdditions[i])) {
        additions.push(rawAdditions[i])
      }
    }
  ...
  readonly property var recordingStreams: {
    ...
    for (var i = 0; i < candidateRecordingStreams.length; i++) {
      try {
        var node = candidateRecordingStreams[i]
        if (node && node.audio) {
          var name = Model.nodeName(node)
          var props = Model.nodeProps(node)
          if (!Model.isInternalAudioNode(name, props))
            list.push(node)
        }
      } catch (_error) { }
    }
  ...
  readonly property var activeRecordingLabels: {
    var raw = Model.uniqueRecordingStreamLabels(recordingStreams)
    var filtered = []
    for (var i = 0; i < raw.length; i++) {
      if (!isIgnoredRecordingLabel(raw[i])) {
        filtered.push(raw[i])
      }
    }
    return filtered
