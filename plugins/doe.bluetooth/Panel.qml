...
Panel {
  ...
  readonly property bool isBtEnabled: !!root.adapter && root.adapter.enabled
  readonly property bool isBtConnected: isBtEnabled && root.connectedCount > 0

  BarIconButton {
    id: button
    anchors.fill: parent
    bar: root.bar
    text: !root.isBtEnabled ? "󰂲" : (root.isBtConnected ? "󰂱" : "󰂯")
    foreground: root.opened
      ? "#00ff88"
      : (button.tooltipHovered
        ? "#ffffff"
        : (root.isBtConnected ? "#00f5d4" : (root.isBtEnabled ? "#7091a4" : "#556d7d")))
    ...
    tooltipText: !root.isBtEnabled
      ...
      : (root.isBtConnected
        ? "Bluetooth: " + root.connectedCount + " Device(s) Connected\nLeft: Open Devices | Right: Toggle Power"
        : "Bluetooth: Enabled (Ready / No Devices)\nLeft: Open Devices | Right: Toggle Power")
