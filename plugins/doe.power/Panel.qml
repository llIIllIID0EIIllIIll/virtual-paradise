...
Panel {
  ...
  readonly property bool isBatteryCritical: !root.batteryCharging && root.batteryFraction <= 0.15
  readonly property bool isBatteryLow: !root.batteryCharging && root.batteryFraction <= 0.25
  ...
  BarIconButton {
    ...
    text: root.showPercentage && !vertical
      ? Math.round(root.batteryFraction * 100) + "% " + (root.isBatteryCritical ? "󰂃" : root.batteryIcon())
      : (root.isBatteryCritical ? "󰂃" : root.batteryIcon())
    slotSize: Style.bar.iconSlot * (root.showPercentage && !vertical ? 2 : 1)
    foreground: root.opened
      ? "#00ff88"
      : (button.tooltipHovered
        ? "#ffffff"
        : (root.isBatteryCritical
          ? "#ff0055"
    ...
    Rectangle {
      id: alertHalo
      visible: root.isBatteryCritical
      anchors.centerIn: parent
      width: parent.width
      height: 24
      radius: 12
      z: -1
      color: Qt.rgba(1.0, 0.0, 0.33, 0.18)
      border.color: "#ff0055"
      border.width: 1

      SequentialAnimation on opacity {
        running: root.isBatteryCritical
