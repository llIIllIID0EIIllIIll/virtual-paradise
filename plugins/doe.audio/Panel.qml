...
Panel {
  ...
  BarIconButton {
    ...
    foreground: root.opened ? "#00ff88" : (button.tooltipHovered ? "#ffffff" : "#00f5d4")
    scale: button.tooltipHovered ? 1.15 : 1.0
    tooltipText: "Audio: " + (root.sink && root.sink.audio ? (root.sink.audio.muted ? "Muted" : Math.round(root.outputVolume * 100) + "%") : "No Output") + "\nLeft: Open Audio Mixer | Right: Toggle Mute | Scroll: Adjust Volume"
